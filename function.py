...
Original file is located at
    https://colab.research.google.com/drive/1NkJuqPmaOEbLG_DnZXx3V25ZSJMqMl3G
"""
...
def hero_attacks(combat_strength, m_health_points):
    ...
    if not isinstance(combat_strength, int) or not isinstance(m_health_points, int):
        raise ValueError("Combat strength or monster health points are not numeric. Please input a number")
...
def monster_attacks(m_combat_strength, health_points):
    if not isinstance(m_combat_strength, int) or not isinstance(health_points, int):
        raise TypeError("Monster combat strength or health points are not numeric . Try inputing a number and not alphabet ")
